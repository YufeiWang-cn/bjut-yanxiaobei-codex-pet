...
当前源码 0.2.6。不熟悉终端的人先看 [零基础安装步骤](BEGINNER.md) 的 C 部分：含打开终端、拖入文件夹、逐条命令和成功标志。也可双击包内 START-HERE.html。任务活动只跟踪 Codex，并保留 `Codex` 来源标识。下面的架构/签名内容保留作进阶参考。
...
燕小北不读取普通 ChatGPT 聊天状态，也不会为了聊天探测申请 macOS“辅助功能”权限。必要目录读取遭拒时只为 Codex 任务和额度所需位置处理权限，不要关闭全局安全功能。
...
- **跟随 Codex 显示/关闭**：新安装默认开启；仅在桌宠已运行时，每 2 秒观察所选应用；观察到 Codex 从运行变为退出后，桌宠进程也会结束。首次登录启动时若 Codex 尚未出现，桌宠保持隐藏并等待；取消勾选后不再跟随生命周期。它不帮你启动 Codex。
...
Mac 只识别 Codex 桌面日志，并从 app-server 返回的近期本机根任务发现会话记录；普通 ChatGPT 聊天不会进入任务列表。无日志降级主要保障 Codex 任务的开始、结束和中止，审批、错误、远程任务或本地文件缺失可能不完整。列表不等于 Codex 侧栏总数。
...
更新检查：启动时检查 GitHub Release，失败会静默重试一次，右键“检查更新”可以主动检查。查询使用 Electron 的系统网络接口并跟随 macOS 系统代理/PAC；新版本可打开网页、忽略该版本或仅暂时忽略。Mac 发布物仍是实验性源码包，不能安全地把源码 ZIP 自动覆盖为已签名 `.app`，因此需按本章重新构建。为避免误触，桌宠菜单不提供卸载入口；需要卸载时双击发布包根目录的 `Uninstall.command`，可选择保留或清除数据，核心目录随后移入废纸篓。**不需要删除 .codex、Codex 的登录或任务。**
