...
当前版本 0.2.3 适配新版 Codex 的结构化审批记录：只有真正需要用户点击的请求才显示“等待批准”，Codex 自动审查期间保持“进行中”；批准或拒绝只解除对应请求，任务结束后恢复空闲。更新后请关闭再启动增强版。本 Windows 包不能在 Mac 运行；Mac 用户请使用 macOS 源码版，详见 docs/MACOS.md。
