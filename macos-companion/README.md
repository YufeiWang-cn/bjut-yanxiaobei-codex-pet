...
当前源码 0.2.5。零基础用户先看 [分步安装指南](../docs/BEGINNER.md)，或双击包根目录 START-HERE.html。任务活动只检测本机 Codex，普通 ChatGPT 聊天不读取、不展示；任务行保留 `Codex` 来源标识。旧 .app 需重新构建才能更新。新安装默认跟随 Codex 显示；打包 .app 首次运行尝试登录启动（系统可能要求授权），源码运行不会设置登录项。右键可关闭跟随、打开“使用说明”、检查更新或卸载并清除本地数据；解压包也包含 Uninstall.command。
