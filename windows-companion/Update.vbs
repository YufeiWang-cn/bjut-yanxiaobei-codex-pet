...
source = fso.BuildPath(fso.GetParentFolderName(WScript.ScriptFullName), "Updater.ps1")
installRoot = fso.GetParentFolderName(source)
If Not fso.FileExists(source) Then source = fso.BuildPath(fso.BuildPath(installRoot, "windows-companion"), "Updater.ps1")
If Not fso.FileExists(fso.BuildPath(installRoot, "VERSION.txt")) Then installRoot = fso.GetParentFolderName(installRoot)
tempScript = fso.BuildPath(fso.GetSpecialFolder(2), "YanXiaoBei-Updater-" & Replace(Replace(CStr(Timer), ".", ""), ",", "") & ".ps1")
